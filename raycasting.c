...
/* if chunk is already loaded, no need to reload everything */
Chunk raycastGetLazyChunk(Map map, int X, int Z)
{
	int cx = ((X - (int) raycast.rasterDest[0]) >> 4) + 1;
	int cz = ((Z - (int) raycast.rasterDest[1]) >> 4) + 1;
	if (0 <= cx && cx < raycast.rasterChunks + 2 &&
	    0 <= cz && cz < raycast.rasterChunks + 2)
	{
		...
		if ((c->cflags & CFLAG_GOTDATA) && c->X == X && c->Z == Z)
		{
			//fprintf(stderr, "reusing chunk already loaded at %d, %d\n", X, Z);
			return c;
		}
	}
	return NULL;
